...
class DenseRetriever(object):
    ...
    # this is how you get the question vectors
    ...
    # now i need to get the question vectors also here
    def get_top_docs(
...
def main(args):
    ...
    questions_tensor_numpy = [t.numpy() for t in questions_tensor]
    # here are the question tensors already!
    zipped = zip(question_answers, questions_tensor_numpy)

    file = args.out_file + ".pkl"
    pathlib.Path(os.path.dirname(file)).mkdir(parents=True, exist_ok=True)
    logger.info("Writing question tensors to %s" % file)
    with open(file, mode="wb") as f:
        pickle.dump(list(zipped), f)
